...
def test_detects_new_sensitive_port_and_service():
    baseline = BehaviorObservation("192.168.1.10", frozenset({80}), frozenset({"http"}), "Linux", 20)
    current = BehaviorObservation("192.168.1.10", frozenset({22, 80}), frozenset({"http", "ssh"}), "Linux", 25)
...
def test_detects_identity_and_exposure_shift():
    baseline = BehaviorObservation("10.0.0.5", frozenset({443}), frozenset({"https"}), "Windows", 10)
