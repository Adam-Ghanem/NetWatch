...
def test_ai_advisor_low_confidence_without_data():
    result = build_ai_advice([], [], [])
...
def test_ai_advisor_prioritizes_high_risk_ports():
    ports = [
    ...
    result = build_ai_advice(hosts, ports, inventory)
...
def test_ai_advice_markdown_contains_sections():
    result = build_ai_advice([], [], [])
    markdown = advice_to_markdown(result)

    assert "NetWatch AI Advisor" in markdown
