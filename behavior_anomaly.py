...
def normalize_observation(value: Mapping[str, object]) -> BehaviorObservation:
    ...
    return BehaviorObservation(
        ...
        services=frozenset(str(item).strip().lower() for item in services if str(item).strip()) if isinstance(services, Iterable) else frozenset(),
...
def detect_behavior_anomalies(
    ...
    if added_ports:
        severity = "high" if any(port in {22, 23, 3389, 445, 5900} for port in added_ports) else "medium"
        findings.append(BehaviorAnomaly(
            current.asset, "new_ports", severity, 0.95,
            (f"new open ports: {', '.join(map(str, added_ports))}",),
        ))
    if removed_ports:
        findings.append(BehaviorAnomaly(
            current.asset, "ports_closed", "low", 0.95,
    ...
    if added_services:
        findings.append(BehaviorAnomaly(
            current.asset, "new_services", "medium", 0.9,
            (f"new services: {', '.join(added_services)}",),
        ))
    if removed_services:
        findings.append(BehaviorAnomaly(
            current.asset, "services_removed", "low", 0.9,
    ...
    if baseline.device_family != current.device_family and current.device_family != "Unknown":
        findings.append(BehaviorAnomaly(
            current.asset, "identity_shift", "high", 0.9,
            (f"device family changed from '{baseline.device_family}' to '{current.device_family}'",),
        ))

    exposure_delta = current.exposure_score - baseline.exposure_score
    if exposure_delta >= 20:
        findings.append(BehaviorAnomaly(
            current.asset, "exposure_increase", "high", 0.9,
            (f"exposure score increased by {exposure_delta} points",),
        ))
    elif exposure_delta <= -20:
        findings.append(BehaviorAnomaly(
            current.asset, "exposure_decrease", "info", 0.85,
