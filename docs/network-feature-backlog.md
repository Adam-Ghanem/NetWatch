...
| P2 | Flow/session fidelity | Flow analysis, correlation, topology, timeline, display filters, Community ID, anomaly modules, bounded direction-aware TCP control history, and conservative handshake/termination-quality evidence exist. | Add sequence-aware retransmission/loss indicators only after capture metadata safely exposes bounded TCP sequence/ack evidence, then add investigator pivots for reset and partial-close sessions. |
