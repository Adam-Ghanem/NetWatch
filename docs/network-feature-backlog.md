...
| --- | --- | --- | --- |
| P0 | Service/version evidence | `port_scanner.py` has bounded SSH, FTP, SMTP, HTTP, and TLS evidence; service history persists normalized product/version confidence. A bounded `tls_service_history` store now retains TLS protocol, cipher, ALPN, SHA-256 certificate fingerprint, and certificate validity metadata across scans without certificate bodies or identity fields. | Add evidence-safe TLS change intelligence for certificate rotation, protocol/cipher downgrade, and expiry risk, then surface it in authenticated service-history APIs/UI. |
...
| P1 | TLS observability | Offline traffic metadata and protocol analysis exist; active HTTPS checks collect negotiated TLS protocol/cipher, bounded ALPN (`h2`/`http/1.1`) evidence, SHA-256 certificate fingerprints, and privacy-preserving certificate validity/status metadata with strict timeout/certificate-size bounds. Normalized TLS history is retained separately from raw certificate material. | Add bounded rotation/downgrade/expiry transitions and investigator pivots without collecting certificate bodies or subject/issuer strings by default. |
