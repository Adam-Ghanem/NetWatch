...
The repository contains an ABC architecture document, a single-tenant fallback overlay, a shared-service Kubernetes reference, durable outbox/job primitives, optional backend adapters, SLO/recovery guidance, report exports, acceptance checklists, a local staging benchmark, and an isolated SQLite snapshot/restore drill. These artifacts strengthen Track A evidence, but they do not replace a real shared database, multi-tenant integration tests, representative deployed load tests, independent security testing, off-host backup rotation, or production restore evidence.
...
| F-03 | A bounded local staging benchmark now covers health, inventory, Markdown/PDF reports, and retention status, but concurrent scan capacity, database stress, and deployed multi-replica scale remain unmeasured. | High | CVSS `N/A`; high operational impact because shared-service capacity and SLO claims cannot be defended with representative evidence. | Locust or k6 scenarios against the target staging topology, fixed environment description, p50/p95/p99 latency, throughput, error rate, saturation point, and before/after regression report. |
...
| F-05 | An isolated local SQLite snapshot/restore drill now passes integrity, schema, readiness, and inventory checks, but off-host encrypted rotation, managed-key recovery, and production RTO/RPO remain unproven. | High | CVSS `N/A`; high availability and integrity impact if a local SQLite file, key, or host is lost. | Encrypted off-host backup job, key-management procedure, restore drill on the target platform, RTO/RPO measurements, retention/deletion evidence, and operator sign-off. |
