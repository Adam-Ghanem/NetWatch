...
MAX_CONCURRENT_CAPTURES = _env_int(
...
HOSTNAME_LOOKUP_ENABLED = _env_bool("NETWATCH_HOSTNAME_LOOKUP_ENABLED", True)
HOSTNAME_LOOKUP_TIMEOUT_SECONDS = _env_int(
    "NETWATCH_HOSTNAME_LOOKUP_TIMEOUT_SECONDS", 1, minimum=1, maximum=3
)
RETENTION_DEFAULT_DAYS = _env_int("NETWATCH_RETENTION_DEFAULT_DAYS", 90, minimum=1, maximum=3_650)
RETENTION_MAX_DELETE_ROWS = _env_int(
    "NETWATCH_RETENTION_MAX_DELETE_ROWS", 10_000, minimum=100, maximum=25_000
)
...
NETWATCH_WEBHOOK_URL = os.getenv("NETWATCH_WEBHOOK_URL", "").strip()
NETWATCH_SLACK_WEBHOOK_URL = os.getenv("NETWATCH_SLACK_WEBHOOK_URL", "").strip()
NETWATCH_NOTIFY_INCLUDE_RAW_TARGETS = _env_bool("NETWATCH_NOTIFY_INCLUDE_RAW_TARGETS", False)
NETWATCH_NOTIFY_DEBOUNCE_SECONDS = _env_int(
    "NETWATCH_NOTIFY_DEBOUNCE_SECONDS", 900, minimum=30, maximum=86_400
)
NETWATCH_NOTIFY_MIN_SEVERITY = os.getenv("NETWATCH_NOTIFY_MIN_SEVERITY", "High").strip().title()
if NETWATCH_NOTIFY_MIN_SEVERITY not in {"Low", "Medium", "High", "Critical"}:
    NETWATCH_NOTIFY_MIN_SEVERITY = "High"
